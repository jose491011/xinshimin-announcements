...
export default function EditAnnouncementClient({ ann }: { ann: Announcement }) {
  ...
  const [form, setForm] = useState({
    notice_number: ann.notice_number,
    title: ann.title,
    category: ann.category as AnnouncementCategory,
    status: ann.status as AnnouncementStatus,
    published_date: ann.published_date.replace(/\//g, '-'),
    summary: ann.summary,
    keywords: ann.keywords.join('，'),
    pdf_url: ann.pdf_url,
    next_step: ann.next_step,
    completion_record: ann.completion_record,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('編輯公告（Prototype）:', form);
    setSaved(true);
    ...
    <AdminLayout>
      <div className="max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-800">編輯公告</h1>
        </div>
