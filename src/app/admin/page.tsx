...
import { announcements, workProgressList, regulations } from '@/data/mockData';
...
export default function AdminPage() {
  ...
  const totalReg = regulations.length;
  ...
  const stats = [
    ...
    { label: '規約辦法',   value: totalReg,  sub: '文件資料夾',            icon: '📜', href: '/admin/regulations',   color: 'bg-amber-50 border-amber-200' },
  ];
    ...
    <AdminLayout>
      <div className="max-w-4xl">
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">管理社區公告、工作進程與規約辦法資料</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link href="/admin/announcements" className="text-sm text-orange-500 font-medium">查看全部 ›</Link>
          </div>
              ...
              <div key={ann.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2 ml-3">
                  ...
                  <Link href={`/admin/announcements/${ann.id}/edit`}>
                    <button className="text-xs text-orange-500 font-medium px-2 py-1 rounded-lg hover:bg-orange-50 transition-colors">編輯</button>
                  </Link>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-6">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <Link href="/admin/progress" className="text-sm text-blue-500 font-medium">查看全部 ›</Link>
          </div>
          <div className="divide-y divide-gray-50">
              ...
              <div key={wp.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex items-center gap-2 ml-3">
                  ...
                  <Link href={`/admin/progress/${wp.id}/edit`}>
                    <button className="text-xs text-blue-500 font-medium px-2 py-1 rounded-lg hover:bg-blue-50 transition-colors">編輯</button>
                  </Link>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <h2 className="text-base font-bold text-gray-700">規約辦法</h2>
            <Link href="/admin/regulations" className="text-sm text-amber-600 font-medium">查看全部 ›</Link>
          </div>
          <div className="divide-y divide-gray-50">
            {regulations.map((reg) => (
              <div key={reg.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 transition-colors">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 truncate">{reg.title}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{reg.tag} · 更新 {reg.updated}</p>
                </div>
                <Link href={`/admin/regulations/${reg.id}/edit`}>
                  <button className="text-xs text-amber-600 font-medium px-2 py-1 rounded-lg hover:bg-amber-50 transition-colors ml-3">編輯</button>
                </Link>
              </div>
            ))}
          </div>
