...
export default function NewRegulationPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('新增規約（Prototype）:', form);
    setSaved(true);
    ...
    <AdminLayout>
      <div className="max-w-2xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-800">新增規約辦法</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="date" value={form.updated.replace(/\//g, '-')}
                onChange={(e) => handleChange('updated', e.target.value.replace(/-/g, '/'))}
                className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
            </div>
          </div>
